...
# Assigns independent MCMC chains to distinct logical cores/workers to speed up execution.
const N_CHAINS = 4
if nprocs() < N_CHAINS
...
const TARGET_COL = "SBP"    # Target variable: Systolic Blood Pressure
...
# Shuffles data and chunks it into `k_folds` partitions for CV.
function load_data_and_create_chunks(file_path, target_col, seed, k_folds)
    ...
    # Distribute the randomized indices evenly among the chunks
    chunks = [shuffled_idx[(i-1)*fold_size + 1 : (i == k_folds ? n : i*fold_size)] for i in 1:k_folds]
    
    return y_raw, X_raw, chunks
end

# Retrieves the Train, Val, and Test splits for a specific fold and standardizes them.
function get_fold_data(y_raw, X_raw, chunks, fold_k, k_folds)
    # The current fold chunk is our strict TEST set
    test_idx = chunks[fold_k]
    # The fold chunk adjacent to the test set acts as the VALIDATION set
    val_idx_k = mod1(fold_k + 1, k_folds)
    val_idx = chunks[val_idx_k]
    
    # All other chunks assemble the TRAINING set
    ...
    # IMPORTANT: We calculate the mean and std ONLY from the Training set 
    # to prevent data leakage into the Validation/Test sets.
    μ_X = mean(X_train, dims=1)
    σ_X = std(X_train, dims=1)
    σ_X[σ_X .< 1e-8] .= 1.0 # Protect against zero-variance features
...
# Helper function to generate credible intervals cleanly
...
# Evaluates the quality of predictions generated by posterior models
function calculate_predictive_metrics(X_test, y_test, β_samples)
    n_samples = size(β_samples, 2)
    n_test = length(y_test)
    
    # Pre-compute centering matrices for Distance Correlation (dCor)
    ...
    for s in 1:n_samples
        ...
        # Distance Correlation (captures non-linear associations)
...
# Calculates Gelman-Rubin (R-hat) and Effective Sample Size (ESS) for MCMC chains
function calculate_mcmc_diagnostics(chain_results)
    n_s = length(chain_results[1]["σ²"])
    # Restructure chain lists into 3D tensors expected by MCMCDiagnosticTools
...
# Master table to hold results for every penalty across every fold
...
for fold_k in 1:NUM_FOLDS
    ...
    # Calculate Lipschitz strictly on training data
    if n_train < p
    ...
    # Evaluate every type of regularizer (Lasso, L1_half, etc.) for this CV fold
    for penalty_name in PENALTY_NAMES
        println("  -> Evaluating Penalty: $penalty_name")
        
        @everywhere model_config = MODELS[$penalty_name]
        
        # Parallel Execution over distinct chains (workers)
        ...
        # Aggregate parallel chains vertically/horizontally
        ...
        # Safe 95% Credible Interval (CI) & Posterior Inclusion Probability (PIP) calculations
        ...
        # Count selected features: Excludes 0 if the credible interval does not cross zero
        is_selected_ci = [(!isnan(l) && (l > 0.0 || u < 0.0)) for (l,u) in zip(lower, upper)]
        model_size_ci = sum(is_selected_ci)
        
        # Count selected features: Inclusion probability > 50%
        is_selected_pip = pip_vec .> 0.5 
        model_size_pip = sum(is_selected_pip)
        
        # Store average coefficients for the final breakdown table
        coef_details_all_folds[penalty_name][:, fold_k] = vec(mean(all_β, dims=2))
        
        # Append rows to the Master tracking table
        push!(fold_metrics, (
            fold_k, penalty_name,
        ...
        # 1. Nullify massive arrays to drop pointers and free memory
        all_β = nothing
        all_σ² = nothing
        chain_results = nothing
        
        # 2. Force Garbage Collection on the Master process to prevent memory leaks over iterations
        GC.gc()
...
# Group performance per-penalty, calculating means and std.devs across folds
...
# Export all metrics and breakdowns cleanly to disk
